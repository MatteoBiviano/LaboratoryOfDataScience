/*
	...
	dal testo del progetto per il cubo costruito all'interno del progetto "group6_cube". 
...
WITH MEMBER grand_total AS
	([Geography_Winner].[Gerarchia].currentmember.parent,[Measures].[N winners distinct])
SELECT {[Measures].[N winners distinct], grand_total} ON COLUMNS,
	([Geography_Winner].[Continent].[Continent], NONEMPTY([Geography_Winner].[Gerarchia].[Country Ioc])) ON ROWS
FROM [Group 6 DB];
...
sum(([Time].[DayMonthQuarterYear].[Year].&[2016]:[Time].[DayMonthQuarterYear].currentmember), 
		[Measures].[Winner Rank Points])
SELECT {[Measures].[Winner Rank Points], running_yearly} ON COLUMNS,
	NONEMPTY([Time].[DayMonthQuarterYear].[Year]) on rows
FROM [Group 6 DB]
WHERE [Geography_Winner].[Continent].&[Europe];
/*
...
*/
WITH MEMBER running_yearly AS
sum(([Time].[DayMonthQuarterYear].currentmember.FIRSTSIBLING:[Time].[DayMonthQuarterYear].currentmember),
		[Measures].[Winner Rank Points])
SELECT {[Measures].[Winner Rank Points], running_yearly} ON COLUMNS,
	NONEMPTY([Time].[DayMonthQuarterYear].[Year]) on rows
FROM [Group 6 DB]
...
Nota: 
	...
	la funzione PARALLELPERIOD, come segue:
		([Time].[DayMonthQuarterYear].currentmember, 
		 [Measures].[Winner Rank Points])/
			(PARALLELPERIOD(
				[Time].[DayMonthQuarterYear].[Year], 1, [Time].[DayMonthQuarterYear].currentmember), 
			[Measures].[Winner Rank Points]))
*/
WITH MEMBER ratio AS
	IIF([Time].[DayMonthQuarterYear].currentmember.prevmember IS NULL, 0, 
	  ([Time].[DayMonthQuarterYear].currentmember, 
	  [Measures].[Winner Rank Points])/([Time].[DayMonthQuarterYear].currentmember.lag(1), 
	  [Measures].[Winner Rank Points]))
SELECT ratio ON COLUMNS,
	[Time].[DayMonthQuarterYear].[Year] ON ROWS
FROM [Group 6 DB];
